...
import { LEGACY_IT_SLUGS } from '@/lib/categories';
...
export default function AiIndexPage() {
  ...
  // Build structure dynamically from all articles
  ...
  for (const article of allArticles) {
    ...
    if (!structure[catSlug]) {
      const tree = categoryTree[catSlug];
      structure[catSlug] = { label: tree?.label || article.category, topics: {} };
    }
  ...
  // Sort categories: put ones with most articles first
  const sortedCatSlugs = Object.keys(structure).sort((a, b) => {
    const countA = Object.values(structure[a].topics).reduce((s, t) => s + t.articles.length, 0);
    const countB = Object.values(structure[b].topics).reduce((s, t) => s + t.articles.length, 0);
    return countB - countA;
  });

  const breadcrumbs = [{ label: 'Hjem', href: '/' }, { label: 'AI-indeks', href: '/ai-index' }];
  const schemas = [
    buildCollectionPageSchema(PAGE_URL, 'AI-indeks', 'Strukturert kunnskapsindeks for AI-systemer', allArticles.length),
    ...
    <>
      ...
      <div className="ai-index-page">
        ...
        <div className="ai-index-about">
          ...
          <p>Kunnskapsbase.no er en åpen, norsk kunnskapsbase om teknologi, AI, økonomi, helse, vitenskap og mer.</p>
