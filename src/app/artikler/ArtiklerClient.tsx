...
export default function ArtiklerClient({ articles }: { articles: ArticleMeta[] }) {
  ...
  const categories = useMemo(() => {
    ...
    for (const a of articles) cats.add(a.category);
    const sorted = Array.from(cats).sort((a, b) => a.localeCompare(b, 'nb'));
    return ['Alle', ...sorted];
  }, [articles]);
  ...
  const date = (d: string) => {
    if (!d) return '';
    const dt = new Date(d);
    return isNaN(dt.getTime()) ? '' : dt.toLocaleDateString('nb-NO', { year: 'numeric', month: 'short', day: 'numeric' });
  };
