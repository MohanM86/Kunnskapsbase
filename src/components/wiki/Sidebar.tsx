...
interface SidebarProps { categoryTree: CategoryTree; activeCategory?: string; }
...
const catIcons: Record<string, React.ReactNode> = {
  ...
  nettsider: (<svg width="14" height="14" viewBox="0 0 14 14" fill="none"><rect x="1" y="1.5" width="12" height="9" rx="1.2" stroke="currentColor" strokeWidth="1.2"/><line x1="1" y1="4.5" x2="13" y2="4.5" stroke="currentColor" strokeWidth="1"/><circle cx="3" cy="3" r="0.6" fill="currentColor"/><circle cx="5" cy="3" r="0.6" fill="currentColor"/><line x1="5.5" y1="13" x2="8.5" y2="13" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/><line x1="7" y1="10.5" x2="7" y2="13" stroke="currentColor" strokeWidth="1.2"/></svg>),
};

export default function Sidebar({ categoryTree, activeCategory }: SidebarProps) {
  const pathname = usePathname();
  const [openCategories, setOpenCategories] = useState<Set<string>>(new Set(activeCategory ? [activeCategory] : []));
  const toggleCategory = (slug: string) => { const next = new Set(openCategories); if (next.has(slug)) next.delete(slug); else next.add(slug); setOpenCategories(next); };
      ...
      {sortedCategories.map(([catSlug, cat]) => {
        ...
        return (
          <div key={catSlug} className="sidebar-section">
            <button className={`sidebar-category ${isOpen ? 'open' : ''} ${activeCategory === catSlug ? 'active-cat' : ''}`} onClick={() => toggleCategory(catSlug)} aria-expanded={isOpen}>
              <span className="sidebar-cat-inner"><span className="sidebar-cat-icon">{catIcons[catSlug]}</span>{cat.label}</span>
              <span className="sidebar-chevron" aria-hidden="true">{isOpen ? '▾' : '▸'}</span>
            </button>
            {isOpen && (
              <div className="sidebar-items">
                {cat.articles.map((article) => { const href = `/${article.slugPath.join('/')}`; return (<Link key={article.slug} href={href} className={`sidebar-link ${isActive(href) ? 'active' : ''}`}>{article.title}</Link>); })}
                {Object.entries(cat.subcategories).map(([subSlug, sub]) => (
                  <div key={subSlug} className="sidebar-subcategory">
                    ...
                    {sub.articles.map((article) => { const href = `/${article.slugPath.join('/')}`; return (<Link key={article.slug} href={href} className={`sidebar-link sidebar-link-sub ${isActive(href) ? 'active' : ''}`}>{article.title}</Link>); })}
                  </div>
                ))}
              </div>
            )}
