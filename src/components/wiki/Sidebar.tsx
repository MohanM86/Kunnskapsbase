...
const catIcons: Record<string, React.ReactNode> = {
  ...
  ai: (
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
      ...
      <circle cx="7" cy="2" r="1.2" stroke="currentColor" strokeWidth="1"/>
      <circle cx="7" cy="12" r="1.2" stroke="currentColor" strokeWidth="1"/>
      <circle cx="2" cy="7" r="1.2" stroke="currentColor" strokeWidth="1"/>
      <circle cx="12" cy="7" r="1.2" stroke="currentColor" strokeWidth="1"/>
      <line x1="7" y1="3.2" x2="7" y2="4.5" stroke="currentColor" strokeWidth="1"/>
      <line x1="7" y1="9.5" x2="7" y2="10.8" stroke="currentColor" strokeWidth="1"/>
      <line x1="3.2" y1="7" x2="4.5" y2="7" stroke="currentColor" strokeWidth="1"/>
      <line x1="9.5" y1="7" x2="10.8" y2="7" stroke="currentColor" strokeWidth="1"/>
    </svg>
  ),
};
...
export default function Sidebar({ categoryTree, activeCategory }: SidebarProps) {
  ...
  // Show all categories that have articles, in a sensible order
  const preferredOrder = ['ai', 'domener', 'dns', 'epost', 'nettsider'];
  const allSlugs = Object.keys(categoryTree);
  const sortedCategories = [
    ...preferredOrder.filter((k) => categoryTree[k]),
    ...allSlugs.filter((k) => !preferredOrder.includes(k)),
          ...
          <div key={catSlug} className="sidebar-section">
            ...
            >
              <span className="sidebar-cat-inner">
                <span className="sidebar-cat-icon">{catIcons[catSlug] || null}</span>
                {cat.label}
              </span>
