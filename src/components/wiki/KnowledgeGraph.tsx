      ...
      <div className="kg-sections">
        {related.length > 0 && (
          <div className="kg-section">
            ...
            <div className="kg-links">
              {related.map((a) => (
                <Link key={a.slug} href={`/${a.slugPath.join('/')}`} className="kg-link kg-link-related">{a.title}</Link>
              ))}
            </div>
          </div>
        )}
        {seeAlso.length > 0 && (
          <div className="kg-section">
            ...
            <div className="kg-links">
              {seeAlso.map((a) => (
                <Link key={a.slug} href={`/${a.slugPath.join('/')}`} className="kg-link kg-link-seealso">{a.title}</Link>
              ))}
            </div>
          </div>
        )}
        {sameTopic.length > 0 && (
          <div className="kg-section">
            ...
            <div className="kg-links">
              {sameTopic.map((a) => (
                <Link key={a.slug} href={`/${a.slugPath.join('/')}`} className="kg-link kg-link-topic">{a.title}</Link>
              ))}
            </div>
          </div>
        )}
