...
export function getCategoryTree(): CategoryTree {
  ...
  const categoryLabels: Record<string, string> = {
    domener: 'Domener', dns: 'DNS', epost: 'E-post', nettsider: 'Nettsider',
  };
  for (const article of articles) {
    ...
    if (article.subcategory) {
      ...
      if (!tree[catSlug].subcategories[subSlug]) {
        tree[catSlug].subcategories[subSlug] = { label: article.subcategory, slug: subSlug, articles: [] };
      }
...
export function getEntityIndex(): EntityIndexEntry[] {
  ...
  return all.map((a) => ({
    title: a.title,
    slug: a.slug,
    url: `${SITE_URL}/wiki/${a.slugPath.join('/')}`,
